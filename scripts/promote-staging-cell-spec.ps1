...
function Get-StagingRecordParts {
    ...
    if ($null -ne $record.cell_spec) {
        $manufacturer = [string]$record.cell_spec.manufacturer.name
        $model = [string]$record.cell_spec.model
        if ([string]::IsNullOrWhiteSpace($model)) {
            $model = [string]$record.cell_spec.name
        }
        $year = [string]$record.cell_spec.year
    }
    else {
...
if ([string]::IsNullOrWhiteSpace($StagingRoot)) {
    $StagingRoot = Join-Path $repoRoot "records\_staging\cell-spec"
}
if ([string]::IsNullOrWhiteSpace($CuratedRoot)) {
    $CuratedRoot = Join-Path $repoRoot "records\cell-spec"
}
