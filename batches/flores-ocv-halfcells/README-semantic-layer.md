...
> **Corpus v2, staged for review.** This directory now holds corpus v2, re-authored on BattINFO's first-class electrode model (BIG-MAP/BattINFO#342). The 319 records published on 2026-08-11 are the v1 shape and are still live and untouched; republishing v2 is a separate, review-gated step. Start with `REVIEW-TABLE-V2.md`.

## What is here

| Path | Contents |
|------|----------|
| `REVIEW-TABLE-V2.md` | **Start here.** Two worked record chains, the open decisions, and the validation summary. |
...
| `records/` | The 323 canonical BattINFO JSON records (source of truth; the working `.battinfo/` workspace is gitignored). |
...
| `bundle/deposit.jsonld` | One combined JSON-LD graph for the whole deposit (301 nodes). |
...
electrode-spec (12)    electrode DESIGNS: active-material kind x source x
   ...
   |                    Cited by cell specs via positive_electrode_spec_id.
...
cell-spec (9)          R2032 coin half-cells, cell_configuration = half_cell.
   |                   Working electrode cites its electrode design (6 of 9).
   v cell_spec_id
...
**Electrode is the coated batch**, one per public label, carrying the label as `batch_id` and the as-built dry thickness. Per-cell figures do not live here: they vary from disc to disc and stay on the tests.
...
- `positive_electrode` is the **working electrode**, described inline (active material, mass fraction, 14 mm diameter, dry thickness where every batch under the spec agrees) and, in six of the nine specs, citing the electrode design it realizes through `positive_electrode_spec_id`.
...
Its *design* polarity is a different statement and is derived from the kind, so the silicon, graphite and Si/Gr designs are negative electrodes - they are anode designs, used here as the working electrode of a half cell. The two facts sit in different places (polarity on the electrode spec, cell role in which holder the electrode occupies) and neither is authored twice.

Three cell specs cite no electrode design: `lnmo/intelligent1`, `lnmo/intelligent2` and `silicon_graphite/intelligent2` each cover two designs, and the reference is single-valued. They name both designs in `specification_comment` instead. The cell-spec IRIs are published and root the identity of every cell, test and dataset, so splitting them was left as a decision for review rather than taken here; see `REVIEW-TABLE-V2.md`, decision D1.

`positive_electrode_basis` is set only where domain-battery curates a positive-electrode class: `lfp` for LFP, `nmc` for NMC111 and NMC532. Graphite, silicon, silicon-graphite and LNMO have no positive-electrode term in the vocabulary (the first three are normally anode materials; LNMO has no term at either polarity), so the field is omitted rather than filled with a value that maps to nothing. In v1 that left the coarse device descriptor empty for four specs; in v2 the chemistry reaches the graph anyway, because the cited electrode spec's own node is typed with its chemistry class (`SiliconBasedElectrode`, `LithiumNickelManganeseOxideElectrode`, ...) stacked with its polarity class.
...
- **Canonical records** (`records/<type>/<type>-<id>.json`) are plain BattINFO JSON. Every record carries an `id` of the form `https://w3id.org/battinfo/<ns>/<uid>`, and cross-links use those IRIs (`manufacturer.id`, `active_material_spec_id`, `electrode_spec_id`, `positive_electrode_spec_id`, `cell_spec_id`, `protocol_id`, dataset `about`).
...
Per-record validation is clean: 323 records, 0 errors, 0 SHACL non-conformances, and 3 warnings that are a validator false positive rather than a record defect (`areal_capacity` is mapped by the emitter but missing from the validator's known-key set; gap E5). The deposit-level RO-Crate gold-standard check reports two classes of issue, unchanged from v1 and neither coming from these records:
...
A fourth is new in v2 and is a coverage gap rather than a wrong statement: the deposit graph promotes standalone material records to first-class nodes but not electrode ones, so all 24 electrode records are missing from `deposit.jsonld` even though each validates and emits on its own. `bundle/deposit-coverage.txt` counts it per type; `READINESS-REPORT.md` records it as gap E2.

## Reproducing

```bash
pip install "git+https://github.com/BIG-MAP/BattINFO.git" pyshacl
python build_records.py      # writes drafts/ and .battinfo/records/ (323 records)
python build_bundle.py       # writes records/ and bundle/
```

The build is reproducible on BattINFO main at commit `63da080` (BIG-MAP/BattINFO#342). Re-running against an existing workspace reports `[unchanged]` for all 228 workspace records and rewrites none of the 95 datasets; a rebuild in an empty workspace reproduces all 323 byte for byte apart from `provenance.retrieved_at`, the build timestamp.

Rebuilding on top of the v1 records (seed `.battinfo/records/` from the committed `records/`, minus the retired material layer) is what produced the reviewable diff in this branch: 289 records come back byte-identical, 9 cell specs change, 25 are new.
...
**These steps describe the v1 upload that has already happened.** They are kept as the recipe; the v2 corpus is not uploaded anywhere yet, and the counts below refresh at republish time along with `description-addendum.md`.

Upload two archives, built from this directory:

```bash
cd batches/flores-ocv-halfcells
zip -r battinfo-records.zip records          # v2: 323 files, ~1.4 MB
zip -r battinfo-bundle.zip  bundle           # v2: 329 files, ~5.9 MB
```

| archive | contents |
|---|---|
| `battinfo-records.zip` | `records/` - the canonical BattINFO JSON records. v2: `material-spec` (1), `electrode-spec` (12), `electrode` (12), `cell-spec` (9), `cell-instance` (95), `test-protocol` (4), `test` (95), `dataset` (95). |
...
**Corpus v2 has not been submitted.** The 21 retired records are still live and stay live until the republish supersedes them (see `superseded/README.md`). The republish is the only step that touches the registry, and it happens after this branch is reviewed.

Each published record has a permanent `w3id.org` identifier. Three worth citing:

| record | identifier | v2 status |
|---|---|---|
...
| LNMO-NMP-1 GITT dataset | `https://w3id.org/battinfo/dataset/09vb-kh3s-aq3q-4s3e` | unchanged |
