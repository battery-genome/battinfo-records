...
This document defines the concrete workflow for reviewing, curating, publishing, and later editing shared BattINFO cell-spec records.
...
2. `records/_staging/cell-spec/*.json` is the shared editorial draft source.
3. `records/cell-spec/<record-id>/record.json` is the canonical shared curated source.
...
2. Shared staging draft in `records/_staging/cell-spec/`
3. Reviewed curated record in `records/cell-spec/<record-id>/record.json`
...
1. Confirm scope
   The record belongs in the shared reusable cell-spec corpus, not only in a personal draft or one-off demo.
2. Confirm identity
...
4. Confirm schema shape
   Validate that the JSON is in a BattINFO-supported authoring or canonical cell-spec shape.
...
```powershell
.\scripts\promote-staging-cell-spec.ps1 -Input records/_staging/cell-spec/<draft>.json
```

If promotion is the acceptance step and you do not want to keep the shared staging copy, use:

```powershell
.\scripts\promote-staging-cell-spec.ps1 -Input <draft>.json -DeleteStagingOnSuccess
```

If the draft is ambiguous, provide an explicit id basis:

```powershell
.\scripts\promote-staging-cell-spec.ps1 -Input <draft>.json -Year 2025
.\scripts\promote-staging-cell-spec.ps1 -Input <draft>.json -Revision sd12
.\scripts\promote-staging-cell-spec.ps1 -Input <draft>.json -EvidenceDate 2026-03-20
```
...
1. Edit `records/cell-spec/<record-id>/record.json`
2. Re-run validation
...
- reviewable shared cell-spec content
