...
- Use lowercase slug ids with `--` between major segments.
- Use single `-` only inside a segment.
...
Use descriptive revision tokens for duplicate review drafts when needed, for example `lg--e66a--misfetched-pdf`.
