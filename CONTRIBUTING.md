...
3. Put a single JSON draft in `records/_staging/cell-types/<submission-name>.json`.
   When the intended curated id is already known, use it as the staging filename too.
4. Use BattINFO repo tooling to validate the staging draft and promote it to `records/cell-types/<record-id>/record.json`.
...
3. Reviewed curated record in `records/cell-types/`.
